...
class ReportsAnalytics extends StatelessWidget {
  ...
  Widget _buildTrendBar(String disease, int cases, double maxCases) {
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("$disease: $cases Cases", style: const TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          Stack(
            children: [
              Container(
  ...
  Widget build(BuildContext context) {
    final int totalPrescriptions = prescriptionReports.fold(0, (sum, item) => sum + (item['value'] as int));
    final double maxCases = diseaseTrends.map((e) => e['monthlyCases'] as int).reduce((a, b) => a > b ? a : b).toDouble();
        ...
        child: Column(
          ...
          children: [
            ...
            ...diseaseTrends.map((disease) =>
                _buildTrendBar(disease['disease'], disease['monthlyCases'], maxCases)
            ),
          ],
        ),
      ),
